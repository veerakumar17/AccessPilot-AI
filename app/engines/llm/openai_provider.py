"""
OpenAI LLM provider.
Used when LLM_PROVIDER=openai in .env (future deployment).
"""
...
import structlog
from openai import AsyncOpenAI, APIConnectionError, APITimeoutError, AuthenticationError, RateLimitError, BadRequestError
...
# ---------------------------------------------------------------------------
# Retry policy — only retry on transient network/rate errors
# ---------------------------------------------------------------------------
...
class OpenAIProvider(LLMClient):
    ...
    Uses the OpenAI API with retry, structured responses, and full error handling.
    """

    def __init__(self) -> None:
        if not settings.openai_api_key:
            raise OpenAIException("OPENAI_API_KEY is not configured")
        self._client = AsyncOpenAI(
            api_key=settings.openai_api_key,
            timeout=settings.openai_request_timeout,
            max_retries=0,  # retries handled by tenacity for full control
        )
        logger.info("OpenAI provider initialised", model=settings.openai_model)
    ...
    async def _complete_with_retry(
        ...
        Raises OpenAIException on permanent errors.
        """
        attempt = 0
        ...
        for attempt in range(1, settings.openai_max_retries + 1):
            try:
                ...
                raise OpenAIException("OpenAI authentication failed. Check your API key.") from exc

            except BadRequestError as exc:
                logger.error("OpenAI bad request", error=str(exc), label=label)
                raise OpenAIException(f"OpenAI rejected the request: {exc.message}") from exc

            except RateLimitError as exc:
                wait = _backoff_seconds(attempt)
                logger.warning("OpenAI rate limited", attempt=attempt, wait_seconds=wait)
                if attempt >= settings.openai_max_retries:
                    raise OpenAIException("OpenAI rate limit exceeded. Please retry later.") from exc
                await asyncio.sleep(wait)

            except APITimeoutError as exc:
                logger.warning("OpenAI request timed out", attempt=attempt, timeout=settings.openai_request_timeout)
                if attempt >= settings.openai_max_retries:
                    raise OpenAIException(f"OpenAI request timed out after {settings.openai_request_timeout}s.") from exc
                await asyncio.sleep(_backoff_seconds(attempt))

            except APIConnectionError as exc:
                logger.warning("OpenAI connection error", attempt=attempt, error=str(exc))
                if attempt >= settings.openai_max_retries:
                    raise OpenAIException("Could not connect to OpenAI API.") from exc
                await asyncio.sleep(_backoff_seconds(attempt))

            except Exception as exc:
                logger.error("Unexpected OpenAI error", error=str(exc), label=label, exc_info=True)
                raise OpenAIException(f"Unexpected error calling OpenAI: {exc}") from exc

        raise OpenAIException("OpenAI request failed after all retries.")
